...
export class RaphApp<Props extends RaphProperties = RaphProperties> {
  ...
  // Константы
  //
  private _maxUps = Raph.MAX_UPS
  private _minUpdateInterval = 1000 / Raph.MAX_UPS
  ...
  run(): void {
    ...
    for (const phase of this._phasesArray) {
      ...
      if ('all' in phase && typeof phase.all === 'function') {
        ...
        Raph.events.emit('nodes:notified', { ctxs: expandedCtxs })
      }
        ...
        for (const ctx of expandedCtxs) {
          phase.each(ctx)
          Raph.events.emit('node:notified', { node: ctx.node, event: ctx.events ?? null })
        }
